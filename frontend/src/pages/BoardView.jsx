...
const SortableList = ({ list, color, children }) => {
  ...
  return (
    <div ref={setSortableNodeRef} style={style} className="flex flex-col w-full md:w-72 flex-shrink-0">
      <div {...attributes} {...listeners} className={`flex flex-col h-full ${color}/90 hover:bg-opacity-100 rounded-lg shadow-lg transition-colors cursor-grab`}>
...
const AddCardForm = ({ listId, onCardCreated }) => {
  ...
  return (<form onSubmit={handleCreateCard}><textarea value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter a title for this card..." className="w-full p-2 rounded-md text-[#212A31] focus:outline-none focus:ring-2 focus:ring-[#124E66] resize-none" autoFocus onBlur={() => setIsEditing(false)} /><div className="mt-2 flex items-center gap-2"><button type="submit" className="bg-[#124E66] hover:bg-opacity-80 text-white font-bold py-2 px-4 rounded-md transition-colors">Add Card</button><button type="button" onClick={() => setIsEditing(false)} className="text-gray-500 hover:text-gray-700"><FiX size={24} /></button></div></form>);
};
const AddListForm = ({ boardId, onListCreated }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [title, setTitle] = useState('');
  const handleCreateList = async (e) => { e.preventDefault(); if (!title.trim()) { setIsEditing(false); return; } try { const { data: newList } = await axios.post('/api/lists', { title, boardId }); onListCreated(newList); setTitle(''); setIsEditing(false); } catch (error) { console.error('Error creating list:', error); } };
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeType === 'Card' && overType === 'List') {
        ...
        setBoard(prev => {
            ...
            const destCards = [...(destList.cards || []), { ...activeCard, list: destListId }];
          ...
          <SortableContext items={board.lists.map(l => l._id)} strategy={horizontalListSortingStrategy}>
            <div className="md:inline-flex md:h-full items-start gap-4 space-y-4 md:space-y-0">
              {board.lists.map((list, index) => (
                <SortableList key={list._id} list={list} color={listColors[index % listColors.length]}>
