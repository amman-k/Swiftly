...
const SortableList = ({ list, color, children, onUpdateListTitle }) => {
  const { setNodeRef, isOver } = useSortable({ id: list._id, data: { type: 'List' } });
  const { attributes, listeners, setNodeRef: setSortableNodeRef, transform, transition } = useSortable({ id: list._id, data: { type: 'List' } });
...
const BoardView = () => {
  ...
  useEffect(() => {
    ...
    socket.on('cardCreated', (data) => handleCardCreated(data.newCard, data.listId));
    socket.on('listsReordered', ({ orderedListIds }) => {
      setBoard(prev => {
        if (!prev) return null;
        const listMap = new Map(prev.lists.map(list => [list._id, list]));
        const newLists = orderedListIds.map(id => listMap.get(id)).filter(Boolean);
        return { ...prev, lists: newLists };
      });
    });

    // --- MODIFICATION: This block is corrected to be stable ---
    socket.on('cardsReordered', ({ listId, orderedCardIds }) => {
      setBoard(prev => {
        ...
        const newLists = prev.lists.map(list => {
          if (list._id === listId) {
            const reorderedCards = orderedCardIds.map(id => allCardsMap.get(id)).filter(Boolean);
    ...
    socket.on('listUpdated', ({ listId, newTitle }) => {
      setBoard(prev => {
        if (!prev) return null;
        const newLists = prev.lists.map(list => list._id === listId ? { ...list, title: newTitle } : list);
        return { ...prev, lists: newLists };
      });
    });
    return () => {
